...
export const api = {
  ...
  update: (
    id: number,
    data: Partial<Pick<Todo, "title" | "done" | "list_id">>
  ) =>
  ...
  lists: {
    ...
    update: (
      id: number,
      data: Partial<Pick<List, "name" | "position" | "color">>
    ) =>
  ...
  notepad: {
    all: () => request<Todo[]>("/api/notepad"),

    create: (title: string) =>
      request<Todo>("/api/notepad", {
        method: "POST",
        body: JSON.stringify({ title }),
      }),
    ...
    moveToList: (id: number, listId: number) =>
      request<Todo>(`/api/todos/${id}`, {
        method: "PATCH",
